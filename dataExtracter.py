...
def main():
   ...
   whiteList = open("Whitelisted Images.txt", 'w')
   blackList = open("Blacklisted Images.txt", 'w')
   dirPath = os.getcwd() #Get the current directory path

   #Make the directories to store the image information
   try:
      if not os.path.isdir("Input Images"):
         os.mkdir("Input Images")
      if not os.path.isdir("Image Masks"):
         os.mkdir("Image Masks")
      if not os.path.isdir("Mask Data"):
         os.mkdir("Mask Data")
      if not os.path.isdir("Mask Validation"):
         os.mkdir("Mask Validation")
      if not os.path.isdir("Blacklist Masks"):
   ...
   for row in reader:
      ...
      newImg.save(dirPath + "/Input Images/" + row['ID'] + ".jpg")
      newImg.close()
      ...
      newMask.save(dirPath + "/Image Masks/" + row['ID'] + "_mask.jpg")
      ...
      cv2.imwrite(dirPath + "/Mask Validation/" + row['ID'] + "_validation_mask.jpg",
                  validationMaskImage)

      maskDataFile = open(dirPath + "/Mask Data/" + row['ID'] + "_mask_data.txt", 'w')
      ...
      if not inValid:
         newMask.save(dirPath + "/Whitelist Masks/" + row['ID'] + "_mask.jpg")
         whiteList.write(row['ID'] + '\n')
      else:
         newMask.save(dirPath + "/Blacklist Masks/" + row['ID'] + "_mask.jpg")
         print("Potential labeling error for image: " + row['ID'])
         blackList.write(row['ID'] + '\n')
